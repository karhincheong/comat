...
def matrix_multiplication(x, y):
    return [[sum(x[i][k] * y[k][j] for k in range(len(x))) for j in range(len(y[0]))] for i in range(len(x))]
...
def matrix_determinant(x):
    if len(x) == 1:
        ...
        return sum(x[0][i] * matrix_determinant(matrix_subtraction(x[1:], [[x[0][j]] for j in range(len(x[0]))])) * (-1)**i for i in range(len(x[0])))
...
def matrix_inverse(x):
    ...
    if det == 0:
        ...
        return [[matrix_determinant(matrix_subtraction(x, [[x[0][j]] for j in range(len(x[0]))])) * (-1)**i for i in range(len(x[0]))] for j in range(len(x))]
